...
describe('resolveSourceScope', () => {
  ...
  it('returns the exact source path for file targets', () => {
    expect(
      resolveSourceScope(resolveQualityTarget(REPO_ROOT, 'src/shared/scope/source.ts'))
    ).toBe('src/shared/scope/source.ts');
  });

  it('returns the package src directory when the target is the src folder itself', () => {
    expect(
      resolveSourceScope(resolveQualityTarget(REPO_ROOT, 'src'))
    ).toBe('src');
  });

  it('returns explicit test-tree targets', () => {
    expect(
      resolveSourceScope(resolveQualityTarget(REPO_ROOT, 'tests'))
    ).toBe('tests');
  });
...
describe('assertSourceScope', () => {
  ...
  it('allows non-source-looking targets because tool includes define source policy', () => {
    expect(assertSourceScope(resolveQualityTarget(REPO_ROOT, 'tests'))).toBe(
      'tests'
    );
  });
});

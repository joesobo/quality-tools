...
describe('resolveQualityTarget', () => {
  ...
  it('resolves explicit source files inside a package', () => {
    ...
    expect(assertSourceScope(target)).toBe('src/cli/crap.ts');
  });
  ...
  it('resolves root files with root package metadata', () => {
    const target = resolveQualityTarget(REPO_ROOT, 'README.md');
    expect(target.kind).toBe('file');
    expect(target.packageName).toBe('quality-tools');
    expect(target.packageRelativePath).toBe('README.md');
    expect(target.relativePath).toBe('README.md');
  });
